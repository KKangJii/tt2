...
<body>
	...
	<div class="container">
		...
		<div>
			...
			<table width="100%" class="type09">
				...
				<c:forEach var="qna" items="${ listqna }" varStatus="loop">
					<tr>
						<td>${ qna.board_no }</td>

						<td><c:forEach var="i" begin="0" end="${ qna.reply }">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						</c:forEach> <c:if test="${ qna.reply ne 0 }">
								<img src="<%=request.getContextPath()%>/img/reply_icon.gif" />
							</c:if> <%-- <a onclick = "doAction('${ board.no}')"> --%>
							 <a href="javascript:doAction('${ qna.board_no}')"> ${ qna.title }
						</a></td>
						<td>${ qna.writer }</td>
						<td>${ qna.reg_date }</td>
					</tr>
		...
		<br/>
		<div align="center">
		<input type="button" class="btn btn-primary btn-lg" value="QnA 등록" onclick="doWrite()" />
		</div>
	...
	<script>
		...
		function doWrite(){
			location.href = "<%=request.getContextPath()%>/qnaForm.do";
		}
	</script>
